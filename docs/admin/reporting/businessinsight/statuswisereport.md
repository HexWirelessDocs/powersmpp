...
## 5. Vendor Wise Partner Report

The **Vendor Wise Partner Report**, available under the *Status-wise Report* section, provides administrators with a consolidated view of gateway-level traffic and its associated user consumption.

This report is designed to help administrators understand how a specific gateway is being utilized across the platform.

### Purpose of the Report

The primary objective of this report is to:

- Display total traffic processed through a selected gateway
- Show the list of users routing traffic via that gateway
- Present total consumption for each user
- Provide visibility for billing and operational review

This allows the administrator to evaluate gateway performance and usage distribution from a single interface.

![Vendor Wise Partner Report](images/statuswise5.png)

---

## 6. Partner Wise Vendor Report

The **Partner Wise Vendor Report**, also available under the *Status-wise Report* section, provides a user-centric view of traffic distribution across multiple gateways.

This report focuses on how a specific user's traffic is routed through different gateways.

### Purpose of the Report

This report allows administrators to:

- Review total usage of a selected user
- Analyze how traffic is distributed across assigned gateways
- Monitor submission counts across multiple routes
- Check status-wise delivery counts per gateway

It provides a complete picture of a user's routing behavior and consumption pattern.
